package src.com.leetcode.s3342;
...
class Solution {
    public int minTimeToReach(int[][] moveTime) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (int[] dir : dirs) {
                ...
                int dist = Math.max(best[x][y], moveTime[newX][newY]) + (step % 2 == 0 ? 1 : 2);
                if (dist < best[newX][newY]) {
                    best[newX][newY] = dist;
                    pq.offer(new int[]{newX, newY, dist, step + 1});
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.minTimeToReach(new int[][]{{0, 0, 0, 0}, {0, 0, 0, 0}}));
